  ...
  return (
    <>
      <div className="min-h-[15rem] md:min-h-[27.25rem] bg-[url('/about/background-jainery.png')] bg-[var(--color-five)] bg-cover px-[2rem] md:px-lr-custom flex items-center justify-center relative">
        <h1 className="text-[var(--color-two)] text-[2.25rem] md:text-[4.69rem] font-bold">Blog Post</h1>
      ...
      <div className="bg-[linear-gradient(#fff,_#fff5ed)] px-[2rem] md:px-lr-custom py-[4rem] md:py-[7.5rem]">
        ...
        <div className="mt-[3rem] md:mt-[5rem] flex flex-col md:flex-row flex-wrap gap-[1.5rem]">
          {products.map((product: Products) =>(
            <div key={product.id} className="min-h-[32rem] p-[1.5rem] bg-[linear-gradient(#fff5ed,_#f9e3d0)] shadow-md rounded-[0.5rem] flex flex-col gap-[1rem] box-border" style={{flexBasis: 'calc(33.33% - 20px)'}}>
              <div className="bg-center bg-cover w-[100%] min-h-[15rem] rounded-[0.5rem]" style={{ backgroundImage: `url(${product.background})` }}></div>
              <h1 className="text-[var(--color-three)] text-[0.9rem] md:text-[1rem] font-semibold mt-[1rem]">{product.date}</h1>
